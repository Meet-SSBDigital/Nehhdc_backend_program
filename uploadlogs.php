    ...
    if (move_uploaded_file($filename, $destination)) {
        ...
        if ($handle) {
                ...
                if (strpos($line, "query :") !== false) {
                    ...
                    if ($type === 'organization') {
                        ...
                        $sql = "INSERT INTO organization_logs
                            ...
                            VALUES (
                                ...
                                " . (int)($jsonData['isactive'] ?? 0) . ",
                                 FROM_UNIXTIME(" . ((int)($jsonData['createdate'] ?? 0) / 1000) . "),
                                FROM_UNIXTIME(" . ((int)($jsonData['updatedate'] ?? 0) / 1000) . "),
                            )";
                        ...
                        // Insert into weaverdetailsmaster_logs
                                    $dob = !empty($jsonData['dob']) ? date('Y-m-d', $jsonData['dob'] / 1000) : null;

                        $sql = "INSERT INTO weavermaster
            ...
            VALUES (
                ...
                '" . mysqli_real_escape_string($conn, $jsonData['KYC_type'] ?? '') . "',
                '" . mysqli_real_escape_string($conn, $jsonData['KYC_id'] ?? '') . "',
                FROM_UNIXTIME(" . ((int)($jsonData['createddate'] ?? 0) / 1000) . "),
                                FROM_UNIXTIME(" . ((int)($jsonData['updateddate'] ?? 0) / 1000) . "),
                              
                '" . mysqli_real_escape_string($conn, $jsonData['email'] ?? '') . "'
            )";
                    } else if ($type === 'productdetailsmaster') {
